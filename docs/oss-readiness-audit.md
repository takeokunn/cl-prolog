...
- a split regression suite under `tests/` with a table-driven expectation
  DSL, runnable with or without ASDF
- an expression/branch coverage gate (`scripts/coverage.lisp`, sb-cover):
  100% on every reachable expression as of 0.2.0
...
- cut a tagged release (v0.2.0) once CI is green on `main`
...
2. `tests.lisp` passes (full suite, including script JSON contracts)
