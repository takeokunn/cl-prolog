...
| Meta-call | `(call g)`, `(once g)`, `(ignore g)`, `(not g)`, `(forall c a)`, `(repeat)` | invoke goals, control their solutions, or generate repeated proofs |
...
| Lists | `(member ?x list)`, `(append ?a ?b ?c)`, `(reverse ?a ?b)`, `(length ?l ?n)` | relations over proper and partially instantiated lists |
| Finite domains | `(#= a b)`, `(#\= a b)`, `(#< a b)`, `(#=< a b)`, `(#> a b)`, `(#>= a b)`, `(in x range)`, `(indomain x)` | constrain integers and enumerate finite-domain solutions |
