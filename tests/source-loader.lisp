...
(progn
  ...
  (:signals (consult-prolog ":- op(500, 42, invalid).")
            "Operator specifiers must be atoms")
  (:signals (consult-prolog ":- op(500, bogus, invalid).")
            "Operator specifiers must name a supported associativity")
  (:signals (%apply-source-directive-for-test (quote (cl-prolog::op)))
            "OP requires three arguments")
  (:signals (%apply-source-directive-for-test (quote (cl-prolog::consult)))
            "CONSULT requires a source")
  (:signals (%apply-source-directive-for-test (quote (cl-prolog::ensure_loaded)))
            "ENSURE_LOADED requires a source")
  (:signals (%apply-source-directive-for-test (quote (cl-prolog::load_files)))
            "LOAD_FILES requires a source")
  (:signals (%apply-source-directive-for-test (quote (cl-prolog::set_prolog_flag)))
            "SET_PROLOG_FLAG requires a flag and value")
  (:signals (%apply-source-directive-for-test (quote (cl-prolog::char_conversion)))
            "CHAR_CONVERSION requires input and output characters")
  (:signals (%apply-source-directive-for-test (quote (cl-prolog::dynamic)))
            "DYNAMIC requires an indicator")
  (:signals (%apply-source-directive-for-test (quote (cl-prolog::multifile)))
            "MULTIFILE requires an indicator")
  ...
  (:signals (consult-prolog ":- module(only_name).")
            "Module declarations require a name and export list")
